...
fn fused_quantize_scale_pack(
    ...
    if bits > 4 {
        return fused_quantize_scale_pack_scalar_body(
            rot_orig,
            rot_calib,
            shift,
            inv_scale_tq,
...
fn fused_quantize_scale_pack(
    ...
    fused_quantize_scale_pack_scalar_body(
        rot_orig,
        rot_calib,
        shift,
        inv_scale_tq,
...
#[inline(always)]
fn quantize_code(value: f32, boundaries: &[f32]) -> u8 {
    if boundaries.len() <= 31 {
        let mut code = 0u8;
        for &b in boundaries {
            if value > b {
                code += 1;
...
#[inline(always)]
fn fused_quantize_scale_pack_scalar_body(
    rot_orig: &[f32],
    rot_calib: &[f32],
    shift: &[f32],
    inv_scale_tq: &[f32],
